...
const NameSearch: React.FC<INameSearchProps> = ({
    ...
    return (
        <div className='name-search' ref={autocompleteRef}>
            <label htmlFor='name-search-bar' className='name-search__label'>
                {ENameSearch.NAME}
            </label>
            <input
                id='name-search-bar'
            ...
            />
            {searchTerm ? (
                <FaTimes className='name-search__icon' onClick={handleClearInput} />
            ) : (
                <FaSearch className='name-search__icon' />
            )}

            {showAutocomplete && (
                <div className='name-search__autocomplete'>
                    {filteredData.map((item, index) => (
                        <div key={index} className='name-search__autocomplete-item' onClick={() => handleSelect(item.name)}>
                            {item.name} - {item.address}, {item.country}
                        </div>
                    ))}
                    {onCreateNew && (
                        <>
                            <hr className='name-search__divider' />
                            <div className='name-search__autocomplete-item' onClick={onCreateNew}>
                                or <span className='name-search__create-new'>{ENameSearch.CREATE_NEW_ENTITY}</span>
                            </div>
                        </>
                    )}
                </div>
